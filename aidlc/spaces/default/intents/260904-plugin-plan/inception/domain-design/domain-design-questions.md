...
## Q7. 取り込みで増える定義（5 段階・Depth 対応表・決めた前提の書き方・ラウンド規則）はどこに置きますか？（要求整理からのやり直し後に追加）

FR8 で、断片（28 ファイル同一のテンプレート）に入る内容が増えます。いまのプラグインは contributions のみ（C1）で、`knowledge/` などは持っていません。

A. 断片テンプレートの中に書く。プラグインは contributions のみのまま、28 断片は同一テンプレートを保つ
B. プラグインに `knowledge/` を足して定義を 1 ファイルに置き、断片からはそれを参照する（manifest に `knowledge` を追加し、C1 の「contributions のみ」を緩める）
C. 定義は AI-DLC 側の `aidlc/spaces/<space>/memory/project.md` に置き、断片は参照だけする（配布物には含まれない）
D. Not yet defined（まだ決めていない）
E. Not applicable
X. Other (please specify)
...
- Q1（インストーラ）= A: 参照先 deep-spec-analysis の `install.ts` を写し、定数（`grilling` / `amadeus-dlc/aidlc-grilling-plugin` / `grilling-install.json`）と deep-spec 固有の後処理だけ差し替える。`install.ts` と `release.ts` の 2 本に分ける（「一旦Aで。後々リファクタリングしよう」）
- Q2（完了案内）= A: 「次の質問ステージのモード選択に Grill me が 4 択目として現れる」ことと、エンジン更新後は `/aidlc plugin sync` を再実行することを表示する
- Q3（リリースツール）= A: 参照先と同じ（事前検査 → manifest 更新 → 英語コミット → タグ → atomic push、`--check-tag` あり）
- Q4（文書の置き場）= A: ルート `docs/` は現状維持（計画書はそこで完了記録化）、`grilling/docs/decisions.md` と `decisions.ja.md` を新設
- Q5（選択キー環境の確認）= A: `grilling/tests/` に opt-in のテストとして置き、結果を decisions に転記する
- Q6（LICENSE）= A: MIT License（参照先と同じ、著作権者も同じ）
- Q7（FR8 の定義の置き場）= A: 断片テンプレートの中に書く。プラグインは contributions のみのまま、28 断片は同一テンプレートを保つ
