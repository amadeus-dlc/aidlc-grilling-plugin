...
//
// Copied from aidlc-workflows/tests/harness/sdk-drive.ts (checkout a277af21,
// v2.7.x) for the grilling plugin's live Claude check. Local changes, kept
// deliberately small so the file can be re-synced from upstream:
//   - SHIPPED_SETTINGS resolves against the sibling aidlc-workflows checkout
//     (AIDLC_WORKFLOWS_CHECKOUT or ../../../aidlc-workflows) instead of a
//     repository-relative dist/, because this file no longer lives inside the
...
// The framework checkout that ships dist/claude (this repository's submodule).
const REPO_ROOT =
  process.env.AIDLC_WORKFLOWS_CHECKOUT?.trim() ||
  join(HARNESS_DIR, "..", "..", "..", "aidlc-workflows");
...
// Markers Claude Code sets in the shell of a running session. Forwarding them
// makes the SDK's child CLI believe it is nested inside that session, so the
// live test strips them (the SDK sets its own CLAUDE_CODE_ENTRYPOINT).
const NESTED_SESSION_ENV = new Set([
  "CLAUDECODE",
...
function processEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (NESTED_SESSION_ENV.has(key)) continue;
    if (typeof value === "string") out[key] = value;
  }
  return out;
}
